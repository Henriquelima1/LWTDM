...
# Função para redimensionar a imagem para dimensões divisíveis por 224
def resize_image(img, target_width, target_height):
    # Redimensionar para as novas dimensões divisíveis por 224
    return img.resize((target_width, target_height), Image.Resampling.LANCZOS)

# Função para cortar a imagem em blocos de 224x224
def cut_image_into_patches(img_path, output_dir, patch_size=224, resize_dims=(2016, 896)):
    # Abrir a imagem
    img = Image.open(img_path)

    # Redimensionar a imagem para dimensões divisíveis por 224
    ...
    patch_number = 0
    # Iterar sobre a imagem e cortar em blocos de 224x224
...
def process_images(input_dir, output_dir, resize_dims=(2016, 896), num_images=25):
...
output_directory = 'dataset_28_224/hr_224'   # Diretório para salvar os blocos 224x224
num_images = 25  # Número de imagens que você deseja processar

# Definir dimensões para redimensionamento (divisíveis por 224)
resize_dims = (2016, 896)  # Dimensões ajustadas para serem divisíveis por 224
